...
pcap_handler = CFUNCTYPE(None, POINTER(c_ubyte), POINTER(pcap_pkthdr),
                         POINTER(c_ubyte))
...
pcap_lookupnet.argtypes = [STRING, POINTER(bpf_u_int32), POINTER(bpf_u_int32),
                           STRING]
...
pcap_next_ex.argtypes = [POINTER(pcap_t), POINTER(POINTER(pcap_pkthdr)),
                         POINTER(POINTER(u_char))]
...
pcap_dump.argtypes = [POINTER(pcap_dumper_t), POINTER(pcap_pkthdr),
                      POINTER(u_char)]
...
pcap_compile.argtypes = [POINTER(pcap_t), POINTER(bpf_program), STRING, c_int,
                         bpf_u_int32]
...
pcap_compile_nopcap.argtypes = [c_int, c_int, POINTER(bpf_program), STRING,
                                c_int, bpf_u_int32]
...
if WIN32:
    ...
    pcap_offline_filter.argtypes = [POINTER(bpf_program), POINTER(pcap_pkthdr),
                                    POINTER(u_char)]
    ...
    pcap_sendqueue_queue.argtypes = [POINTER(pcap_send_queue),
                                     POINTER(pcap_pkthdr), POINTER(u_char)]

    # u_int pcap_sendqueue_transmit (pcap_t *p, pcap_send_queue *queue, int sync)
    # 	Send a queue of raw packets to the network.
    pcap_sendqueue_transmit = _lib.pcap_sendqueue_transmit
    pcap_sendqueue_transmit.retype = u_int
    pcap_sendqueue_transmit.argtypes = [POINTER(pcap_t),
                                        POINTER(pcap_send_queue), c_int]

    # int pcap_findalldevs_ex (char *source, struct pcap_rmtauth *auth, pcap_if_t **alldevs, char *errbuf)
    # 	Create a list of network devices that can be opened with pcap_open().
    pcap_findalldevs_ex = _lib.pcap_findalldevs_ex
    pcap_findalldevs_ex.retype = c_int
    pcap_findalldevs_ex.argtypes = [STRING, POINTER(pcap_rmtauth),
                                    POINTER(POINTER(pcap_if_t)), STRING]
    ...
    pcap_parsesrcstr.argtypes = [STRING, POINTER(c_int), STRING, STRING, STRING,
                                 STRING]
    ...
    pcap_open.argtypes = [STRING, c_int, c_int, c_int, POINTER(pcap_rmtauth),
                          STRING]
    ...
    pcap_remoteact_accept.argtypes = [STRING, STRING, STRING, STRING,
                                      POINTER(pcap_rmtauth), STRING]
...
class WinPcapDevices(object):
    ...
    def __enter__(self):
        ...
        all_devices = ctypes.POINTER(pcap_if_t)()
...
# class MulticastReceiverProtocol:
